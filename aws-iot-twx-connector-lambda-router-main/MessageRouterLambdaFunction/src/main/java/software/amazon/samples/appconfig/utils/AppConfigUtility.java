...
/**
 * Handles AppConfig data access from the cache or from a network request
 *
...
public class AppConfigUtility {
    ...
    private static final Logger logger = LoggerFactory
            .getLogger(AppConfigUtility.class + "::LAMBDA_BODY");
     ...
     * @param clientId           unique clientId that is sent to the AppConfig API. Default is a
     *                           random UUID.
     */
     ...
     *
     * @param configurationKey specifies the application name, environment name, and the
     *                         configuration name of the configuration to be
     *                         *                         retrieved.
     * @return GetConfigurationResponse Containing the content, content type, and version of the
     * configuration.
     */
    public GetConfigurationResponse getConfiguration(final ConfigurationKey configurationKey) {
        final ConfigurationCacheItem<GetConfigurationResponse> result =
                Optional.ofNullable(cache.get(configurationKey)).map(item -> {
                    if (item.isRefreshNeeded()) {
                        return getConfigurationFromApiAndApplyToCache(
                                configurationKey,
                                item,
                                item.getValue()
                        ...
                        () -> getConfigurationFromApiAndApplyToCache(configurationKey, null, null));
    ...
    protected ConfigurationCacheItem<GetConfigurationResponse> getConfigurationFromApiAndApplyToCache(
        ...
        try {
            ...
            final GetConfigurationResponse result = client.getConfiguration(
            ...
            final ConfigurationCacheItem<GetConfigurationResponse> item
                    = new ConfigurationCacheItem<>(cacheItemTtl);
            ...
            if (existingItem == null || existingItem.getValue() == null
                    || (item.getValue() != null
                    && !existingItem.getValue().configurationVersion()
                                    .equals(item.getValue().configurationVersion()))) {
